...
SELF_PLAY = False   # True：AI vs AI; False：人机（用 input 读玩家）

NULLMOVE = False  # False比True的棋子显著更高
QS = True
MAX_DEPTH = 20 # 受THINK_TIME限制，实际最大一般仅为5左右。
MIN_DEPTH = 1 # 如果增大，则THINK_TIME不能过低，否则就可能超时报错。
...
# THINK_TIME前N步线性增长
RAMP_THINK_TIME = True   # 程序开始时该参数为真，则启用前N步爬升
RAMP_N = 6
...
def get_think_time(step, max_time=THINK_TIME, enabled=RAMP_THINK_TIME, N=RAMP_N):
    """
    step: 从0开始计数
    需求：前N步最大思考时间按 1/N,2/N,...,N/N 线性增长到 max_time
    """
    if not enabled:
    ...
    if k >= N:
        return float(max_time)
    return float(max_time) * (k / float(N))
...
class Position(namedtuple('Position', 'board score turn version')):
    ...
    def set(self):
        ...
        # ---- Precompute constants used by value() a lot (speed) ----
        my_sum = sumall[self.version][self.turn]
        op_sum = sumall[self.version][not self.turn]

        if my_sum:
            self.possible_che = self.covered * di[self.version][self.turn]['R' if self.turn else 'r'] / my_sum
            self.bing_prob = di[self.version][self.turn]['P' if self.turn else 'p'] / my_sum
        else:
            self.possible_che = 0.0
            self.bing_prob = 0.0

        if op_sum:
            self.possible_che_opponent = self.covered_opponent * di[self.version][not self.turn]['r' if self.turn else 'R'] / op_sum
        else:
            self.possible_che_opponent = 0.0

        # rooted cache (optional but recommended)
        self._rooted_cache = None
    ...
    def rooted_cached(self):
        c = getattr(self, "_rooted_cache", None)
        if c is not None:
            return c
        c = self.rooted()
        self._rooted_cache = c
        return c
    ...
    def value(self, move):
        ...
        # ---- Use precomputed constants (speed) ----
        possible_che = self.possible_che
        possible_che_opponent = self.possible_che_opponent
        bing_possibility = self.bing_prob
        # Actual move
...
def _di_signature(version: int):
    """
    为 di[version] 计算一个稳定签名：
    di[version][True] 与 di[version][False] 都是 {piece_char: count}
    """
    # 为避免 dict 顺序影响，按 key 排序后转 tuple
    r = tuple(sorted(di[version][True].items()))
    b = tuple(sorted(di[version][False].items()))
    return (r, b)
...
class Searcher:
    def __init__(self):
        ...
        self.start_time = 0.0
        self.time_limit = 0.0
        self._avg_sig = {}   # version -> signature
    ...
    def quiescence(self, pos, moves, oppo):
        ...
        oppo_rooted_set = {254-x for x in oppo_rooted_set}
        ...
        for i, j in moves:
            ...
            k = 254 - j
            if q in 'rcnabpk':
                s += pst_local[q.upper()][k]
            elif q in 'defghi':
                s += avg[oppo.version][oppo.turn][False]
            elif q == 'u':
                s += avg[oppo.version][oppo.turn][True][k]
    
            if j in oppo_rooted_set:
                if p in 'RCNABPK':
                    s -= pst_local[p][j]
                elif p in 'EFGHI':
                    s -= avg[pos.version][pos.turn][False]
    
            if s > maxscore:
                argmax = (i, j)
                maxscore = s
    
        if argmax and board[argmax[1]] == 'c' and (argmax[1] & 15) == 7 and pos.kongtou_score_opponent > 0 and pos.kongtoupao_opponent > 0:
    ...
    def alphabeta(self, pos, alpha, beta, depth, root=True, nullmove=False, nullmove_now=False):
        ...
        if nullmove_now and depth > 3 and not root and any(c in pos.board for c in 'RNCI'):
            opp = get_oppo()
            if all(opp.board[m[1]] != 'k' for m in opp.gen_moves()):
        ...
        if depth == 0:
            if QS:
                score = self.quiescence(pos, moves, get_oppo())
        ...
        for move in [killer] + moves:
            ...
            child = pos.move(move)  # 只做一次
        
            if best == -MATE_UPPER:
                val = -self.alphabeta(child, -beta, -alpha, depth - 1,
                                      root=False, nullmove=nullmove, nullmove_now=nullmove_now)
            else:
                # PVS 窄窗
                val = -self.alphabeta(child, -alpha - 1, -alpha, depth - 1,
                                      root=False, nullmove=nullmove, nullmove_now=nullmove_now)
                if alpha < val < beta:
                    # 需要时再全窗
                    val = -self.alphabeta(child, -beta, -alpha, depth - 1,
            ...
            if val >= MATE_UPPER:
                updated = child.nullmove()  # 复用 child
    ...
    def search(self, pos, history=(), time_limit=None):
        """ Iterative deepening MTD-bi search """
        self.nodes = 0
        # --- calc_average: only when di changes ---
        try:
            sig = _di_signature(pos.version)
        except Exception:
            # 极端情况下（di 结构变化/pos.version 不可用）退回每次计算
            sig = None
        
        if not hasattr(self, "_avg_sig"):
            self._avg_sig = {}
        
        if sig is None:
            self.calc_average(pos.version)
        else:
            last = self._avg_sig.get(pos.version)
            if last != sig:
                self.calc_average(pos.version)
                self._avg_sig[pos.version] = sig
            else:
                # di 未变：直接复用全局缓存 average[version]
                # calc_average 本来也会写 average[version]，这里确保 self.average 指向它
                if pos.version in average:
                    self.average = average[pos.version]
        
        pos.set()

        
        # 用“从头算的粗评估”作为搜索基准分，避免每回合基准乱跳
        pos = Position(pos.board, pos.score_rough, pos.turn, pos.version).set()
        
        # print("AI rough eval: %d" % pos.score_rough)
        ...
        # -----------------------------
        # Time control (NEW)
        # 关键：把时间限制下沉到 alphabeta 内部做“可中断检查”
        # 这里仅负责设置起始时间与上限，并在超时后用“上一层已完成结果”返回
        # -----------------------------
        self.start_time = time.time()
        self.time_limit = THINK_TIME if time_limit is None else float(time_limit)
        ...
        for depth in range(MIN_DEPTH, MAX_DEPTH + 1):
            try:
                ...
                val = self.alphabeta(pos, lower, upper, depth,
                      nullmove=NULLMOVE, nullmove_now=NULLMOVE)

                mv = self.tp_move.get(pos)
                sc = val
    
                # 记录“已完成的一整层”的最佳结果：超时后用它作为最终落子
                if mv is not None:
                    last_move, last_score, last_depth = mv, sc, depth
    
                yield depth, mv, sc
    
            except TimeoutError:
                # 超时：不要等这一层跑完，直接返回上一层已完成的结果
                if last_move is not None:
                    yield last_depth, last_move, last_score
                return
...
def side_to_move_in_check(pos) -> bool:
    # 当前走子方的王/将是大写 'K'
    # 对手的攻击检测：把局面 rotate，让对手成为走子方
    opp = pos.rotate()
    return any(opp.board[m[1]] == 'k' for m in opp.gen_moves())
...
def main(random_move=False, AI=True, debug=False):
    ...
    def pick_ai_move(pos, step):
        """在当前pos视角下选一步。返回 (depth, move, score, nodes)"""
        _depth = 0
        move, score = None, 0
        nodes = 0
        if random_move:
            move = random_policy(pos)
            return _depth, move, 0, 0
    
        if pos.board in kaijuku:
            move = kaijuku[pos.board]
            return _depth, move, 0, 0
    
        tlim = get_think_time(step)   # NEW
        start = time.time()
        generate_forbiddenmoves(pos, check_bozi=True, step=step)
        
        last_depth, last_move, last_score = 0, None, 0
        for d, mv, sc in searcher.search(pos, hist, time_limit=tlim):   # NEW
            last_depth, last_move, last_score = d, mv, sc
            if time.time() - start > tlim:   # NEW
                break
    
        nodes = getattr(searcher, "nodes", 0)
        return last_depth, last_move, int(last_score), int(nodes)
    ...
    while True:
        ...
        if SELF_PLAY and (not debug):
            _depth, move, score, nodes = pick_ai_move(hist[-1], step)

            # 无子可走：直接结束
            if move is None or move not in genmoves:
                dump_scores()
                print("You lost", flush=True)
                break

            # 关键：为了让 UI 的 MOVE_RE 抓到“上一手”，红方也打印 My move
            print("Think depth: {} My move: {} (AI stage score {}) nodes {}".format(
                _depth, render(move[0]) + render(move[1]), score, nodes
                ), flush=True)
        ...
        pos, win, eat, dst = hist[-1].mymove_check(
            move,
            discount_red=True,
            discount_black=SELF_PLAY   # 自我对抗时双方都应更新暗子集合
        )
        ...
        # 此时 hist[-1] 是“电脑走子方视角”（电脑将走）
        # 如果电脑此刻被将军，则输出提示
        if side_to_move_in_check(hist[-1]):
            print("Check!", flush=True)

        # Fire up the engine to look for a move.
        _depth = 0

        move, score = None, 0  # 确保每轮都有初值
        nodes = 0
        if AI:
            if random_move:
                ...
                if hist[-1].board in kaijuku:
                    ...
                    score = 0   # 开局库没有搜索分数
                    # 若你更想给开局库也一个“粗评估”，可用下面一行替换 score=0：
                    # score = int(hist[-1].score_rough)
                else:
                    tlim = get_think_time(step)   # NEW
                    start = time.time()
                    generate_forbiddenmoves(hist[-1], check_bozi=True, step=step)
                    for _depth, move, score in searcher.search(hist[-1], hist, time_limit=tlim):  # NEW
                        if time.time() - start > tlim:   # NEW
                            break
                    nodes = getattr(searcher, "nodes", 0)
            ...
            while move not in genmoves:
                ...
                if match:
                    ...
                    score = 0  # 人工输入不需要 score
                else:
                    print("Please enter a move like h2e2")

        #  只要 AI 这回合最终确实走出一步，就记录一条 score
        if AI and move is not None:
            score_list.append(int(score))
        ...
        print("Think depth: {} My move: {} (AI stage score {}) nodes {}".format(
            _depth, render(254 - move[0]) + render(254 - move[1]), score, nodes
            ))
        pos, win, eat, dst = hist[-1].mymove_check(move)

        if win:
            dump_scores()
            print("You lose, HAHAHAHAHAHAHAHAHAHA!", flush=True)
            break
